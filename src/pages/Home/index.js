...
const Page = () => {
  // Appel à useData() pour obtenir les données
  const { data } = useData();

  // Vérifie si les données existent et si la liste d'événements n'est pas vide
  const last =
    data && data.events && data.events.length > 0
      ?
      // Utilise la méthode reduce pour trouver la prestation la plus récente
      ...
      :
      // Si les données sont absentes ou la liste d'événements est vide, définissez last sur null
      null;
  return (
    <>
      <header>
